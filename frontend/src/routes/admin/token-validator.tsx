...
import { Cancel01Icon, CheckmarkCircle02Icon, Shield01Icon } from "@hugeicons/core-free-icons"
...
function TokenValidatorPage() {
  ...
  const [result, setResult] = React.useState<TokenVerificationResult | null>(null)
  ...
  const data: Array<TableRowData> = React.useMemo(() => {
    ...
    return result.payload.data.map((row, index) => ({ ...row, _rowNum: index + 2 }))
  }, [result])
        ...
        <CardContent>
          <div className="flex flex-col gap-4">
            ...
            <div className="flex gap-2">
              <Button onClick={handleVerify} disabled={!token.trim() || isVerifying}>
        ...
        <>
          <Card className="mb-6">
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                {result.valid ? (
                  <>
                    <HugeiconsIcon icon={CheckmarkCircle02Icon} className="size-5 text-green-600" strokeWidth={2} />
                    <span>Token Valid</span>
                  </>
                ) : (
                  <>
                    <HugeiconsIcon icon={Cancel01Icon} className="size-5 text-red-600" strokeWidth={2} />
              ...
              {result.valid && result.payload ? (
                <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
                  <div>
                    ...
                    <p className="text-lg font-semibold">{result.payload.metadata.source}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="text-lg font-semibold">{result.payload.metadata.row_count}</p>
                  </div>
                  <div>
                    <p className="text-sm font-medium text-muted-foreground">Validated At</p>
                    <p className="text-lg font-semibold">
                      {new Date(result.payload.metadata.validated_at).toLocaleString()}
                    </p>
                  </div>
                </div>
              ) : (
                <p className="text-red-600">{result.error ?? "Verification failed"}</p>
              )}
            </CardContent>
          </Card>
          ...
          {result.valid && result.payload && (
            <>
              ...
              <Card>
                <CardHeader>
                  <CardTitle>Data ({result.payload.data.length} rows)</CardTitle>
                </CardHeader>
                <CardContent>
                  <DataTable
                    data={data}
